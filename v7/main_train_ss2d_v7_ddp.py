"""
SS2D-ViT 학습 스크립트 v7 — DDP (2x RTX TITAN)
  - 단일 GPU v7 스크립트 (main_train_ss2d_v7.py) 의 DDP 버전
  - effective BATCH_SIZE = BATCH_SIZE * world_size (16 * 2 = 32)
  - LR 은 v7 config 의 값을 그대로 사용 (linear scaling 은 첫 run 후 조정)
...
def run_val_ddp(model, val_loader, criterion_l1, device, world_size):
    model.eval()
    all_ssim, all_psnr, all_nmse, all_l1, n_samples = [], [], [], [], 0
    with torch.no_grad():
    ...
    # rank-local sums, then all-reduce to global mean
...
def main():
    ...
    _print('====================================================')
    _print(f' [SS2D-ViT v7 DDP] world_size={world_size}  effective_BS={BATCH_SIZE * world_size}')
    _print('====================================================')
    ...
    ss2d_decoder = DDP(ss2d_decoder, device_ids=[local_rank], find_unused_parameters=False)
    ...
    if is_main:
        wandb.init(
            ...
            name=f'SS2D_v7_ddp_ws{world_size}_BS{BATCH_SIZE}_LR{LEARNING_RATE_ADAM}_EP{NUM_EPOCHS}',
            config={
                'model': 'SS2D-ViT-v7-DDP',
    ...
    if run_baseline:
        ...
        baseline = run_val_ddp(ss2d_decoder, val_loader, criterion_l1, device, world_size)
        best_val_ssim = baseline['ssim']
        if is_main:
            tqdm.write(f'  [Baseline]  SSIM={baseline["ssim"]:.4f} PSNR={baseline["psnr"]:.2f} '
                       f'NMSE={baseline["nmse"]:.4f} L1={baseline["l1"]:.4f}')
    ...
    for epoch in epoch_bar:
        ...
        if do_val:
            ...
            val_metrics = run_val_ddp(ss2d_decoder, val_loader, criterion_l1, device, world_size)
            if is_main:
        ...
        if is_main and (epoch + 1) % 5 == 0:
            torch.save(ss2d_decoder.module.state_dict(),
                       os.path.join(PATH_FOLDER, f'ss2d_vit_epoch_{epoch+1}.pt'))
            tqdm.write(f'  Checkpoint: ss2d_vit_epoch_{epoch+1}.pt')
