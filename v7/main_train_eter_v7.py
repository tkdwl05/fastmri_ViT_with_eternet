"""
ETER-ViT 학습 스크립트 v7 (single-GPU, 24GB VRAM, scratch)
  - Config:     v7/configs/myConfig_choh_ETER_model_v7
  - Dataloader: dataloaders/dataloader_h5_v5 (v5/v6 와 동일)
  - Model:      models/hybrid_eternet/u_choh_model_ETER_ViT_v5.choh_Decoder3_ETER_v5 (v6 와 동일)
  - 변경점 (v6 대비):
      * BATCH_SIZE 4 → 16
      * GRU hidden 6 → 10 (원본 ETER-Net 기본값 복원)
      * RESUME_CKPT 폐기 (scratch)
      * PYTORCH_CUDA_ALLOC_CONF 환경변수 제거
      * num_workers 4 → 16, prefetch_factor 2 → 4
"""
...
# Sanity-test override: SANITY_NUM_EPOCHS=1 python v7/main_train_eter_v7.py
...
def main():
    print('====================================================')
    print(' [ETER-ViT v7] scratch + BS 16 + GRU hidden 10 (capacity 복원)')
    print('====================================================')
    ...
    if not check_env_for_model('eter', 'myConfig_choh_ETER_model_v7', strict=True):
        return
    ...
    wandb.init(
        ...
        name=f'ETER_v7_scratch_BS{BATCH_SIZE}_LR{LEARNING_RATE_ADAM}_EP{NUM_EPOCHS}',
        config={
            'model': 'ETER-ViT-v7',
