"""
SS2D-ViT 학습 스크립트 v7 (single-GPU, 24GB VRAM, scratch)
  - Config:     v7/configs/myConfig_choh_SS2D_model_v7
  - Dataloader: dataloaders/dataloader_h5_v5 (v5/v6 와 동일)
  - Model:      models/hybrid_eternet (ViT 인코더) + hybrid_ss2d/u_choh_model_SS2D_ViT_v4 (디코더, v6 동일)
  - 변경점 (v6 대비):
      * BATCH_SIZE 4 → 16 (24GB VRAM)
      * RESUME_CKPT 폐기 (scratch 학습, v6 ckpt 가 새 머신에 없음)
      * PYTORCH_CUDA_ALLOC_CONF 환경변수 제거
      * num_workers 4 → 16, prefetch_factor 2 → 4 (24 core CPU + HDD I/O 마진)
      * baseline val 은 RESUME_CKPT is None 일 때 skip
"""
...
sys.path.append(os.path.join(_PROJECT_ROOT, 'models', 'hybrid_eternet'))     # choh_ViT
...
# Sanity-test override: SANITY_NUM_EPOCHS=1 python v7/main_train_ss2d_v7.py
...
def main():
    print('====================================================')
    print(' [SS2D-ViT v7] scratch + BATCH_SIZE 16 + skimage SSIM val')
    print('====================================================')
    ...
    if not check_env_for_model('ss2d', 'myConfig_choh_SS2D_model_v7', strict=True):
        return
    ...
    if RESUME_CKPT and os.path.exists(RESUME_CKPT):
        ...
        print("\nScratch training (RESUME_CKPT is None)")
        run_baseline = False
    ...
    val_loader = DataLoader(
        choh_data_val, batch_size=4, shuffle=False,
    ...
    wandb.init(
        ...
        name=f'SS2D_v7_scratch_BS{BATCH_SIZE}_LR{LEARNING_RATE_ADAM}_EP{NUM_EPOCHS}',
        config={
            'model': 'SS2D-ViT-v7',
