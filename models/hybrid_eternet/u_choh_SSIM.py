...
def create_window(window_size, channel=1):
    ...
    # window.cuda()
    window = window.cuda()
    print(' create_window window.device {}'.format(window.device))
    return window
...
def ssim(img1, img2, window_size=11, window=None, size_average=True, full=False, val_range=None):
    ...
    if window is None:
        ...
        print(' window.device {}'.format(window.device))
...
def msssim(img1, img2, window_size=11, size_average=True, val_range=None, normalize=False):
    device = img1.device
    weights = torch.cuda.FloatTensor([0.0448, 0.2856, 0.3001, 0.2363, 0.1333]).to(device)
