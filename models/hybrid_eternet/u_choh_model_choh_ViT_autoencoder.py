...
### one input one output
class choh_Decoder2_with_skip_tail(nn.Module):
	def __init__(
		self,
		*,
		encoder,
		...
		decoder_heads = 8,
		decoder_dim_head = 64,
		decoder_dim_final_linear = 4
	):
		super().__init__()
		print('   \'choh_Decoder2_with_skip_tail    @u_choh_model_choh_ViT_autoencoder\'   ')
		...
		dim_for_final_linear = decoder_dim_final_linear * encoder.patch_size[0] * encoder.patch_size[1]
		self.final_linear = nn.Linear(decoder_dim, dim_for_final_linear)
		kernel_size = 3
		num_ch_tail = decoder_dim_final_linear + 32
		self.tail = nn.Conv2d( in_channels=num_ch_tail, out_channels=1, kernel_size=kernel_size, padding=(kernel_size//2), bias=True)


	def forward(self, in_imgs):
		device = in_imgs.device

		patches = rearrange(in_imgs, 'bb cc (nh p1) (nw p2) -> bb (nh nw) (p1 p2 cc)', p1 = self.encoder.patch_size[0], p2 = self.encoder.patch_size[1])
		...
		# print(' decoded_tokens.shape {}'.format( decoded_tokens.shape ))
		pred_latent_pixels = self.final_linear(decoded_tokens)
		# print(' pred_latent_pixels.shape {}'.format( pred_latent_pixels.shape ))

		x = rearrange(pred_latent_pixels, 'bb (nh nw) (cc p1 p2) -> bb cc (nh p1) (nw p2)', nh=self.encoder.num_patch_h, nw=self.encoder.num_patch_w,p1=self.encoder.patch_size[0], p2=self.encoder.patch_size[1])
		# print(' after rearrange x.shape {}'.format( x.shape ))
		x = torch.cat((x, in_imgs), dim=1)
		# print(' after concat x.shape {}'.format( x.shape ))
...
### one input one output
class choh_Decoder2_with_upsample_tail(nn.Module):
	def __init__(
		self,
		*,
		encoder,
		...
		decoder_heads = 8,
		decoder_dim_head = 64,
		decoder_out_ch_final_linear = 4,
		decoder_out_feat_size_final_linear = 32 #2**5=32
	):
		super().__init__()
		print('   \'choh_Decoder2_with_upsample_tail   @u_choh_model_choh_ViT_autoencoder\'   ')
		...
		self.decoder_out_ch_final_linear = decoder_out_ch_final_linear
		self.decoder_out_feat_size_final_linear = decoder_out_feat_size_final_linear
		dim_for_final_linear = decoder_out_ch_final_linear * decoder_out_feat_size_final_linear * decoder_out_feat_size_final_linear
		self.final_linear = nn.Linear(decoder_dim, dim_for_final_linear)


		# scale = int( math.log(encoder.patch_size[0], 2) - math.log(decoder_out_feat_size_final_linear, 2))
		# self.tail = nn.ModuleList(nn.Sequential(
		# 	Upsampler(scale, n_feats=decoder_out_ch_final_linear),
		# 	nn.Conv2d( in_channels=decoder_out_ch_final_linear, out_channels=1, kernel_size=3, padding=1, bias=True)
		# ))	


		# self.tail = nn.ModuleList()
		# for ss in range(  int( math.log(encoder.patch_size[0], 2) - math.log(decoder_out_feat_size_final_linear, 2))  ):
		# 	self.tail.append(Upsample(decoder_out_ch_final_linear))
		self.tail = []
		for ss in range(  int( math.log(encoder.patch_size[0], 2) - math.log(decoder_out_feat_size_final_linear, 2))  ):
			self.tail.append(Upsample(decoder_out_ch_final_linear))
		self.tail = nn.Sequential(*self.tail)

		self.last = nn.Conv2d( in_channels=decoder_out_ch_final_linear, out_channels=1, kernel_size=3, padding=1, bias=True)


		# self.tail = nn.Sequential(
		# 	nn.Conv2d( in_channels=decoder_out_ch_final_linear, out_channels=4*decoder_out_ch_final_linear, kernel_size=3, padding=1, bias=True),
		# 	nn.PixelShuffle(2),
		# 	nn.Conv2d( in_channels=decoder_out_ch_final_linear, out_channels=1, kernel_size=3, padding=1, bias=True)
		# )
	...
	def forward(self, img):
		...
		# print(' decoded_tokens.shape {}'.format( decoded_tokens.shape ))
		pred_latent_pixels = self.final_linear(decoded_tokens)
		# print(' pred_latent_pixels.shape {}'.format( pred_latent_pixels.shape ))	###(bb, 144, cc 32 32)

		# x = rearrange(pred_latent_pixels, 'b (nh nw) (cc p1 p2) -> b cc (nh p1) (nw p2)', nh=self.encoder.num_patch_h, nw=self.encoder.num_patch_w,p1=self.encoder.patch_size[0], p2=self.encoder.patch_size[1])
		# x = rearrange(x, 'bb (cc ih iw) -> bb cc ih iw', ih = self.image_size[0], iw = self.image_size[1])
		# print(' after rearrange x.shape {}'.format( x.shape ))
		x = rearrange(pred_latent_pixels, 'b (nh nw) (cc p1 p2) -> b cc (nh p1) (nw p2)', nh=self.encoder.num_patch_h, nw=self.encoder.num_patch_w, p1=self.decoder_out_feat_size_final_linear, p2=self.decoder_out_feat_size_final_linear)
		# print(' after rearrange x.shape {}'.format( x.shape ))
...
### one input one output
class choh_Decoder2_with_tail(nn.Module):
	def __init__(
		self,
		*,
		encoder,
		...
		super().__init__()
		print('   \'choh_Decoder2_with_tail @u_choh_model_choh_ViT_autoencoder\'   ')
		...
		self.final_linear = nn.Linear(decoder_dim, dim_for_final_linear)
		kernel_size = 3
		self.tail = nn.Conv2d( in_channels=decoder_dim_final_linear, out_channels=1, kernel_size=kernel_size, padding=(kernel_size//2), bias=True)
...
class choh_ViT(nn.Module):
	def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
		super().__init__()
		print('   \'choh_ViT @u_choh_model_choh_ViT_autoencoder\'   ')
