"""
FastMRI brain multicoil H5 데이터로더 (v4: Data Consistency용 mask + sens 추가 반환)

v3 대비 추가 반환 키:
  - mask: (1, H, W) float — 샘플링 마스크 (sampled=1, zeroed=0)
  - sens: (2*N_COIL_CH, H, W) float — ACS 저주파 영역에서 추정한 sensitivity map
          RSS 기준 unit 정규화, real/imag 교번 배치 (pack_complex_to_real)
...
class FastMRI_H5_Dataloader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ── v4: DC block용 mask / sens 생성 ──
        # mask: (1, H, W) — 샘플링된 phase-encoding 라인만 1
        mask_out = np.broadcast_to(
            mask_1d[np.newaxis, :], (self.N_OUTPUT, self.N_OUTPUT)
        )[np.newaxis].astype(np.float32)  # (1, N, N)

        # sens: ACS 저주파 영역만 남긴 k-space를 iFFT → per-coil 저해상도 복소 이미지
        #       RSS로 픽셀별 정규화하여 Σ|s|²=1 유지 (coil-combined 연산에서 스케일 보존)
        ...
        acs_img = ifft2c(acs_ksp)  # (N_COIL_CH, N, N) complex
        rss_acs = np.sqrt(np.sum(np.abs(acs_img) ** 2, axis=0, keepdims=True))  # (1, N, N)
        rss_acs = np.maximum(rss_acs, 1e-9)
        sens_c  = acs_img / rss_acs  # (N_COIL_CH, N, N) complex
        sens_packed = self._pack_complex_to_real(sens_c)  # (2*N_COIL_CH, N, N) float
