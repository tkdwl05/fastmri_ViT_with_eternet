...
SS2D-ViT 학습 스크립트
  - 인코더: choh_ViT (원본 그대로)
  - 디코더: choh_Decoder_SS2D_ViT (GRU → SS2D 교체)
"""
...
def main():
    ...
    if not check_env_for_model('ss2d', 'myConfig_choh_SS2D_model', strict=True):
        return
    ...
    wandb.init(
        ...
        name=f'SS2D_BS{BATCH_SIZE}_LR{LEARNING_RATE_ADAM}_EP{NUM_EPOCHS}',
        config={
            'model': 'SS2D-ViT',
    ...
    for epoch in epoch_bar:
        ...
        for sample in batch_bar:
            ...
            with torch.amp.autocast("cuda"):
                out = ss2d_decoder(data_in_img, data_in)
