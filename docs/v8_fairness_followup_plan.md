...
## 비교 기준점 명문화 (2026-09-02, 사용자 지적 반영)

**기준점(reference)은 교수님 원본 ETER-Net(GRU, 668M)이다 — SS2D 가 아니다.**
- 모든 팔(SS2D·Transformer·pixel-GRU)은 교수님 골격(out_ch=2×H2=20 원본 상수, UNet DFU,
  dual-input concat [51], 행→열 스캔 축, GRU-계보 레시피)에 꽂히는 **치환 후보**다.
- **1차 보고 축 = 각 팔 vs 원본 GRU** ("원본 대비 치환 이득표"). 팔 간 pairwise(예: pixel-GRU
  vs SS2D 의 메커니즘 해석)는 2차 축 — 분석 스크립트·표·서사 모두 이 순서를 따른다.
- 신규 팔의 예산을 SS2D 스택(~0.1M)에 맞춘 것은 SS2D 가 표준이어서가 아니라 **"가중치 공유
  파라미터화의 최소 공통 예산"** 규약이기 때문(원본은 하한 63M 이라 하향 매칭 불가 — 상향
  매칭은 구조적 비효율의 복제라 무의미). 문구에 주의: SS2D 는 "현재 선두(incumbent)"이지
  "기준점"이 아니다.
