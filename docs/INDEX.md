...
> 본 학습 / 평가 코드의 변천 흐름은 v4 → (v5: 미완료) → v6 → v7 → v7_titan 로 이어집니다. v7_titan 이 현재 운영 버전 (TITAN RTX x2, 384×384, ViT-Base, ETER U-Net 후처리 복원, brain mask 평가). **v8_eter_pure** 는 v7_titan 과 평행한 별도 갈래 — ViT 자체를 뺀 교수님 순수 ETER-Net 위에서 GRU↔SS2D 만 바꾸는 통제비교(no-DC 쌍 완주 = 최종; **DC 축 폐기** — 비표준 확장, `v8_eter_pure_rnn_vs_ss2d.md` §7).
