...
| **2026-08-07** | **[../paper/draft_ko_v1.md](../paper/draft_ko_v1.md)** | **논문 한국어 초안 v1 (교수님 상의용)** — 스코프 v8 통제비교+v9 unleashed, ETER-Net 계열(Oh2020→**Oh2025 ViT-BiRNN, J.Imaging — 문헌검색으로 확인된 교수님 신작**) 직접 후속 포지셔닝. 참고문헌 18건, 투고처 5곳 평가(1위 Bioengineering SI, 2위 Sensors), 우위 슬라이스 비율(probabilistic index)·p<0.001 표기, **결과 수치는 표준 지표만(composite 제거, 08-07 결정)**. 기준선 U-Net/E2E-VarNet 평가 도구 `v8_eter_pure/eval_paired_baselines.py` 준비(GPU 풀런은 radapt 완주 후) |
...
> 본 학습 / 평가 코드의 변천 흐름은 v4 → (v5: 미완료) → v6 → v7 → v7_titan → v8_eter_pure → **v9_mamba** 로 이어집니다. v7_titan(384, ViT-Base, ETER U-Net 후처리 복원, brain mask 평가) 과 v8_eter_pure(ViT 없는 순수 ETER-Net, GRU↔SS2D 통제비교 — no-DC SS2D 완승 = 그 비교의 최종; **DC 축 폐기** — 비표준 확장, `v8_eter_pure_rnn_vs_ss2d.md` §7) 는 완주. **현재 운영 버전은 v9_mamba** — v8 승자 SS2D 를 강화(게이팅·3블록·병목해제, fp16 스캔+ds=3)해 unleashed(R4 품질)→radapt(R 일반화) 순차 학습(`v9_mamba_unleashed_and_radapt.md`).
