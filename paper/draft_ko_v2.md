...
- **GRU arm**: ETER-Net 원본 [1]의 양방향(수평+수직) GRU, 행/열 flatten-reshape 구조. hidden
  배수는 원 코드의 두 설정(canonical `model.py` H=12 / model3 config H=10) 중 **10** 을 채택해
  총 668M. canonical H=12 로는 880.5M — 재현 클래스를 원본 클래스와 동일 세팅에서 파라미터
  완전 일치로 검증했다(2026-09-02). 즉 H=10 채택은 GRU 를 더 작게 잡는 보수적 선택이며, 본문의
  파라미터 격차 21×는 canonical 기준(28×)의 하한이다.
- **SS2D arm (통제판)**: 4방향 selective scan(SS2D [31]) 단일 블록. 출력 채널을 GRU arm 과
  동일한 20으로 강제 정합해 용량 상한을 GRU 이하로 억제 — 총 31.2M(이 중 공유 U-Net 이 31.1M 로
  지배적이고, SSM 스택 자체는 실측 0.12M).
...
- 본 비교는 재귀 **메커니즘** 일반의 열위를 주장하지 않는다 — 원 bi-GRU 는 flatten-reshape
  **파라미터화**(한 줄 12,288차원 입력·hidden 384 양자화·총 파라미터 하한 63M)와 얽혀 있어 두
  효과가 분리되지 않는다. 재귀에 SS2D 와 같은 공간 가중치 공유를 준 pixel-scan bi-GRU(스택
  ~0.1M, 예산 동급)와의 분리 비교를 진행 중이다(2026-09-02 착수).
...
3. **radapt(R 일반화) 포함 여부** — 08-18 학습 재개, **~08-25 완주 예상**. 포함 시
   ...
   질문의 RNN-시대 해법이므로, radapt(mask-conditioning+multi-AR+DC)를 그 SSM-시대 직접 후속으로 서술.
   **→ 결정(2026-09-02, 내부): 편입 방향으로 진행** — radapt 완주 후 R-sweep(R∈{2,4,6,8}) 이
...
5. **단일 시드 한계** — 리뷰 방어용 멀티시드 재학습(비용 큼) 필요성 판단. **외부 검토 의견
   ...
   **→ 결정(2026-09-02, 내부): 축약판 실행 확정** — SEED 인프라·런처 준비 완료
   (`docs/v8_fairness_followup_plan.md` E1), radapt 완주 후 GPU 큐.
...
8. **(09-02 신규) GRU hidden 배수 선택 — 10 vs 12** — 교수님 코드의 두 설정이 다름:
   canonical `model.py` = **12**(총 880.5M) / model3 config = **10**(총 668.2M, v8 채택·config
   주석에 문서화). 재현 클래스는 H=12 세팅에서 원본 클래스와 파라미터 완전 일치 검증(09-02).
   확인 요청: ① 어느 값을 "원본"으로 인용할지(§3.2 는 현재 양쪽을 병기하고 10 채택을 명시),
   ② canonical 충실성이 중요하면 H=12 검증 런(≈6일, VRAM 확인 필요) 추가 여부. 참고: H=10
   채택은 GRU 를 작게 잡는 보수적 방향이라 파라미터 격차 21×는 canonical 기준(28×)의 하한 —
   비교의 유불리 문제는 아님.
