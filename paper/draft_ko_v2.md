...
**[문단 ① — MRI 의 임상 가치와 가속의 필요성]**
MRI 는 연조직 대조도가 뛰어난 비침습·무전리 영상 기법이지만, k-space 를 한 줄(phase-encoding
line)씩 채우는 순차 수집 구조 탓에 스캔 시간이 수 분에서 수십 분에 이른다. 긴 스캔은 환자 부담과
모션 아티팩트, 장비 처리량 저하로 직결되며, 이 때문에 k-space 의 일부만 수집하는 가속
(언더샘플링) 촬영과 그로부터의 영상 복원은 MRI 물리·신호처리의 오랜 핵심 문제였다. 딥러닝
재구성은 이미 전향적 임상 검증 — 무릎 MRI 에서 표준 촬영과의 진단 호환성 입증 [43], 촬영 시간을
절반으로 줄인 전향적 가속 임상 연구 [44] — 을 통과하고 상용 장비에 탑재되는 단계에 이르렀고,
가속 상한을 탐구하는 연구도 이어지고 있다 [45]. 즉 문제의 임상적 가치는 확립되어 있으며, 관건은
어떤 모델 구조가 이 역문제를 더 효율적으로 푸는가이다.

**[문단 ② — 고전 접근: 병렬영상과 압축센싱]**
나이퀴스트 조건을 어긴 언더샘플링은 aliasing 을 낳고, 그 복원은 미지수가 방정식보다 많은
ill-posed 역문제다. 고전적 해법은 두 가지 사전지식에 의존했다. 다중 코일의 공간 감도를 이용하는
병렬영상 — SENSE [4], GRAPPA [5] — 은 임상 표준이 되었으나 가속률이 커지면 노이즈 증폭(g-factor)
이 급격해진다. 압축센싱(CS-MRI) [6]은 무작위 샘플링과 희소성 정규화로 더 높은 가속의 이론적
근거를 제시했으나, 반복 최적화의 재구성 시간과 정규화 파라미터 민감성이 임상 보급의 병목이었다
[24].
...
MoDL [10]이 원형이고, 코일 감도까지 종단 학습하는 E2E-VarNet [11]이 사실상의 표준 baseline 이다
[12]. 둘째, **직접 도메인 변환(direct mapping) 계열**은 센서(k-space) 도메인에서 이미지
...
2. **DC 무관성 실증**: DC 가 전혀 없는 세팅에서의 완승으로, 선행 dead-heat 에 대한 "SS2D 는 DC
   ...
   것이 unrolled 문헌의 DC 관행 [8–16]과 구조적으로 다름을 정리하고 no-DC 설계를 정당화한다.
...
"cornerstone"이다. 반면 본 골격(거대 bi-RNN/SSM 도메인 변환 + 종단 U-Net)은 unroll 반복이
...
| Unrolled + DC | [8–16] | 반복 내부의 작은 정규화 unit | 매 반복 interleave | 구조가 달라 직접 비교 대상 아님 (§5-3) |
...
| 규모 | train 4,108 파일 / 65,028 슬라이스 · val 464 파일 / 7,334 슬라이스 (공식 train/val 분할) |
...
반복에 interleave 되는 구조 [8–16]로, 본 골격의 종단 single soft-DC 와 다르며, (c) 내부
