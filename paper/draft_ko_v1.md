...
(best masked SSIM 0.9140 vs 0.9126, 내부 선택지표 composite 0.9200 vs 0.9182),
...
epoch 당 학습시간을 단축(2.78→2.51 h/ep)하면서 최종 품질을 추가 개선했다(composite 0.9203,
...
- **선행 내부 실험(동기)**: ViT 인코더 하이브리드[3] 위에서 GRU(+U-Net 후처리)와
  SS2D(+DC)를 비교했을 때 dead-heat(masked composite 동률 0.9127)였으나, 이 비교는 DC 유무와
  후처리 구조가 얽힌 confound 를 가진다. 본 연구는 confound 를 제거한 순수 골격에서 질문을
  격리한다: **"도메인 변환 자리에서 Mamba 는 GRU 보다 나은가?"**
- **기여**:
  1. ETER-Net 골격에서 시퀀스 모델만 교체한 **단일 변수 통제 비교** — SS2D 가 5지표 전부,
...
- **composite = 0.5·SSIM + 0.3·(PSNR/40) + 0.2·(1−NMSE)** 는 문헌 표준이 아닌 **본 프로젝트
  자체 설계의 모델 선택용 스칼라**다(가중치는 실용적 선택 — 근거·설계 과정은 내부 문서
  `docs/eval_metric_redesign.md`). 논문에서는 **model-selection criterion 으로만 서술**하고
  결과 판단은 표준 지표로 세운다 — 본 연구의 결론은 composite 없이 개별 표준 지표 전부에서
  동일하게 성립한다(§5.1). 결과표의 composite 열 유지 여부는 상의 포인트(부록 B-6).
- 모델 선택(best ckpt)·조기 종료 기준 = val composite.
...
| | epoch | composite | SSIM | PSNR(dB) | NMSE | L1 | params |
|---|---:|---:|---:|---:|---:|---:|---:|
| **SS2D** | 48 | **0.9200** | **0.9140** | **35.16** | **0.0039** | **8.931** | **31M** |
| GRU | 50 | 0.9182 | 0.9126 | 35.03 | 0.0040 | 9.054 | 668M |

- **matched-epoch 전 구간 우위(wire-to-wire)**: 25개 val 지점(ep2~50) 전부에서 SS2D ≥ GRU
  (Δcomposite +0.0009~+0.0058). ViT 하이브리드 트랙에서 관찰됐던 후반 역전(crossover) 없음.
...
| composite | 0.9087±0.0596 | 0.9104±0.0596 | **76.6%** | <0.001 |
...
| | composite | SSIM | PSNR | NMSE | L1 |
|---|---:|---:|---:|---:|---:|
| **강화 SS2D (best ep78)** | **0.9203** | **0.9145** | **35.18** | 0.0039 | 8.931 |
| 통제판 SS2D (best ep48) | 0.9200 | 0.9140 | 35.16 | 0.0039 | 8.931 |
| GRU (best ep50) | 0.9182 | 0.9126 | 35.03 | 0.0040 | 9.054 |

- per-slice 우위 슬라이스 비율: vs 통제판 SS2D **54~56% (5지표 전부, Wilcoxon p<0.001 —
  유의하나 근소)**, vs GRU **78~82% (완승)**.
- **정직한 해석 (반드시 유지)**: matched-ep50 시점 강화판 = 0.9171 로 통제판 best(0.9200)에
  미달. 0.9200 최초 도달은 ep70. 즉 "같은 학습량에서 더 좋다"가 아니라 **"ep 당 속도 이득
  (2.51 vs 2.78 h/ep)으로 더 긴 스케줄(80ep)을 소화해 최종 품질을 넘었다"**가 정확한 서사
  (best 도달 wall-clock 은 강화판 ≈181h > 통제판 ≈133h). 아키텍처 강화 자체의 순수 이득은 근소.
...
(5지표 mean±std + 본 모델들과의 우위 슬라이스 비율)를 여기에 삽입한다.
...
6. **composite 노출 수위** — 자체 설계 지표(§3.3)라 결과표에 열로 유지할지, 모델 선택
   기준으로만 본문 언급하고 표에서는 뺄지(리뷰어 질문 소지 최소화). 표준 지표만으로도
   결론 동일하므로 빼는 쪽이 방어적.
