"""
v9 언리시드 Mamba(SS2D) — trainer (R4 고정 품질 최대화).

`v8_eter_pure/main_train_pure_v8.py` 클론. 차이:
  1. 모델 = PureETER_SS2D_V9 고정 (GRU/DC 분기·env var 제거).
  2. Optimizer no-WD 파라미터 그룹 (Mamba A_log/D/dt_proj.bias 는 weight_decay 제외 = 정석 위생).
  3. 완료 신호 = DONE sentinel 파일 (v8 supervisor 의 append-log stale-'학습 완료' 오탐 버그 회피).
그 외(true-resume full-state, atomic save, masked L1+SSIM loss, composite/EarlyStop, NaN-skip 가드,
W&B online)는 그대로 재사용. loss = masked L1 + (1 - masked SSIM), best/EarlyStop = masked composite.

실행: python v9_mamba_unleashed/main_train_ss2d_v9.py
      (supervisor: SMOKE_BS/ACCUM_STEPS/SANITY_* env override 가능)
"""
...
from myConfig_ss2d_v9 import *                 # noqa: F401,F403
...
def build_optimizer(model):
    """Mamba 위생: A_log/D/dt_proj.bias(=_no_weight_decay 플래그) 는 weight_decay 제외."""
    decay, no_decay = [], []
    ...
    print(f"   optimizer: Adam  decay={len(decay)} params (wd={LAMBDA_REGULAR_PER_PIXEL})  "
          f"no_decay={len(no_decay)} params (A_log/D 등)")
    return opt
...
def main():
    print('====================================================')
    print(f' [v9 SS2D unleashed]  BS={BATCH_SIZE}  ACCUM={ACCUM_STEPS}  '
          f'(eff_bs={BATCH_SIZE*ACCUM_STEPS})')
    ...
    if not check_env_for_model('ss2d', 'myConfig_ss2d_v9', strict=True):
        return

    # 새 학습 시작 시 stale DONE 제거 (재개면 last.pt 존재 → 아래에서 처리)
    ...
    wandb.init(
        ...
        config={
            'track': 'v9_mamba_unleashed', 'variant': 'unleashed',
    ...
    # 완료 sentinel: supervisor 가 이 파일 존재로 정상 완료 판정 (append-log stale 오탐 회피)
