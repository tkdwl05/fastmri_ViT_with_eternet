...
from myDataloader_fastmri_brain_randomR8_251007 import choh_fastmri_brain_hybrid_ifft_acs16R8_train_v4_1_random
...
def main():
	...
	if flag_torchinfo:
		import torchinfo
		print('\ntorchinfo')
		torchinfo.summary(choh_decoder, input_size=[(4, 32, 384, 384), (4, 32, 384, 384)])
	...
	if flag_torchviz:
		...
		print('\nflag_torchviz')
		x1 = torch.randn(1, 32, 384, 384, requires_grad=True, device='cuda')
		x2 = torch.randn(1, 32, 384, 384, requires_grad=True, device='cuda')
		y = choh_decoder(x1, x2)                 # y가 텐서 하나라면 바로 가능
		if isinstance(y, (list, tuple)):  # 출력이 여러 개인 경우 하나로 합치기
			y = sum(t if t.ndim == 0 else t.sum() for t in y)
		dot = make_dot(y, params=dict(choh_decoder.named_parameters()))
		# ── 가독성 향상 옵션 ─────────────────────────────────────────
		dot.graph_attr.update(
			# rankdir='LR',   # 좌→우 방향 배치(세로 스크롤 덜함)
			dpi='400',      # PNG 렌더링 시 해상도(300~600 권장)
			ranksep='0.35', # 레벨 간 간격
			nodesep='0.25'  # 노드 간 간격
		)
		dot.node_attr.update(
			fontsize='10',  # 노드 라벨 글꼴 크기
			shape='box'     # 박스형 노드(가독성↑)
		)
		dot.edge_attr.update(
			fontsize='8'    # 엣지 라벨 글꼴 크기
		)

		# ── 벡터 포맷(권장) ──────────────────────────────────────────
		dot.render("model_graph_new", format="svg")  # 브라우저에서 확대해도 선명
		dot.render("model_graph_new", format="pdf")  # 인쇄/공유용
	...
	for epoch in range(num_epochs):
		for n_set in range(NUM_TRAIN_SET):
			...
			choh_data_train = choh_fastmri_brain_hybrid_ifft_acs16R8_train_v4_1_random(idx_start= NUM_TRAIN_VOLUME_PER_SET*n_set , idx_end=NUM_TRAIN_VOLUME_PER_SET*n_set+NUM_TRAIN_VOLUME_PER_SET)
