...
def main(args):
	...
	inputs_ksp = np.reshape(inputs_ksp, [len(choh_data_test), 16*2*384, 384])
	results = np.reshape(results, [len(choh_data_test), N_OUT_X, N_OUT_Y])
	refs = np.reshape(refs, [len(choh_data_test), N_OUT_X, N_OUT_Y])
	print('\ninputs.shape {} \ninputs_ksp.shape {} \nresults.shape {} \nrefs.shape {}\n'.format(inputs.shape,inputs_ksp.shape, results.shape, refs.shape))
	...
	while flag_while:
		...
		in_ksp = np.squeeze( inputs_ksp[idx,0:384,:]) ### start:stop:step
		...
		plt.subplot(4,2,5)
		plt.imshow(in_ksp, aspect='equal', cmap=flag_cmap)
		plt.title('in_ksp')
		plt.colorbar()
