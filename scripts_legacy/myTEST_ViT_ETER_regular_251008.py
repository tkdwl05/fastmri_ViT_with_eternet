...
def main(args):
	...
	with torch.no_grad():
		...
		for i_batch, sample_batched in enumerate(testloader):
			...
			reconstruction_times.append(elapsed_time_ms)
	...
	avg_time = np.mean(reconstruction_times)
	max_mem_mb = torch.cuda.max_memory_allocated() / (1024 ** 2) # MB
	print(f'\nPerformance Metrics:')
	print(f'Average per-slice reconstruction time: {avg_time:.4f} ms')
