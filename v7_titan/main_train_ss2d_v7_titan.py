"""
SS2D-ViT 학습 스크립트 v7_titan (single-GPU, TITAN RTX 24GB, scratch, 384×384)
  - Config:     v7_titan/configs/myConfig_choh_SS2D_model_v7_titan
  - Dataloader: dataloaders/dataloader_h5_v5 (target_size=384)
  - Encoder:    choh_ViT (ViT-Base: dim=768, L=12, H=12, MLP=3072)
  - Decoder:    u_choh_model_SS2D_ViT_v4.choh_Decoder_SS2D_ViT
                  (d_state=32, d_inner=128, out_ch=32)
  - 변경점 (v7 대비):
      * IMAGE_SIZE 320→384
      * ViT-Small → ViT-Base
      * SS2D capacity 증대 (d_state 16→32, d_inner 64→128, out_ch 20→32)
...
def save_checkpoint_atomic(obj, path):
    """tmp 파일에 쓴 뒤 os.replace 로 원자적 교체 — 저장 도중 GPU/프로세스가 죽어도
    기존 last.pt 가 깨지지 않도록 보장 (멀티데이 학습 중단 내성의 핵심)."""
    tmp = path + '.tmp'
    torch.save(obj, tmp)
    os.replace(tmp, path)
...
def skimage_ssim_batch_masked(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> float:
    ...
    compare_ssim 의 data_range 는 mask 안의 max-min 으로 산정.
    full SSIM map 을 받아 mask 안의 픽셀만 평균 (배경 부풀림 차단).
    """
...
def main():
    print('====================================================')
    print(f' [SS2D-ViT v7_titan] scratch + 384x384 + ViT-Base + SS2D(d_state=32, d_inner=128, out=32)  BS={BATCH_SIZE}')
    print('====================================================')
    ...
    if not check_env_for_model('ss2d', 'myConfig_choh_SS2D_model_v7_titan', strict=True):
        return
    ...
    # optimizer/scheduler/scaler 가 아직 생성 전이므로, 여기선 model weight 만 load 하고
    # full-state 복원(optimizer/scheduler/scaler/counter/RNG)은 그들이 생성된 뒤에 수행.
    last_ckpt_path = os.path.join(PATH_FOLDER, 'ss2d_vit_last.pt')
    _full_state = None
    run_baseline = False
    if os.path.exists(last_ckpt_path):
        _full_state = torch.load(last_ckpt_path, map_location=device)
        ss2d_decoder.load_state_dict(_full_state['model'])
        resume_mode = 'full'
    ...
    wandb.init(
        ...
        name=f'SS2D_v7_titan_scratch_BS{BATCH_SIZE}_LR{LEARNING_RATE_ADAM}_EP{NUM_EPOCHS}',
        config={
            'model': 'SS2D-ViT-v7_titan',
    ...
    for epoch in epoch_bar:
        ...
        # ── true-resume full-state (rolling, atomic) — 매 epoch 갱신, GPU 다운 내성 ──
        save_checkpoint_atomic({
            'epoch': epoch + 1,                       # 완료한 epoch 수 (재개 시 start_epoch)
            'model': ss2d_decoder.state_dict(),
        ...
        }, os.path.join(PATH_FOLDER, 'ss2d_vit_last.pt'))
