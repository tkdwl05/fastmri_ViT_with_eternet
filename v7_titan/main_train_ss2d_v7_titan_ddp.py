"""
SS2D-ViT 학습 스크립트 v7_titan — DDP (2x TITAN RTX 24GB, scratch, 384×384)

  - Single-GPU 버전 (main_train_ss2d_v7_titan.py) 의 DDP 변환
  - effective BATCH_SIZE = BATCH_SIZE * world_size (default 4 * 2 = 8)
  - LR 은 config 값 그대로 유지 (linear scaling 은 first-run 결과 보고 별도 조정)
  - 평가/loss 는 brain_mask + weighted composite 동일 적용

실행:
  torchrun --nproc_per_node=2 v7_titan/main_train_ss2d_v7_titan_ddp.py
"""
...
def run_val_ddp(model, val_loader, device, world_size):
    ...
    # all_reduce: [s_ssim, n_ssim, s_psnr, s_nmse, s_l1, n_other]
    local = torch.tensor(
...
def main():
    ...
    _print('====================================================')
    _print(f' [SS2D-ViT v7_titan DDP] world_size={world_size}  '
    ...
    if is_main and not check_env_for_model('ss2d', 'myConfig_choh_SS2D_model_v7_titan', strict=True):
    ...
    ss2d_decoder = DDP(ss2d_decoder, device_ids=[local_rank], find_unused_parameters=True)
    ...
    optimizer = torch.optim.Adam(
        ss2d_decoder.parameters(), lr=LEARNING_RATE_ADAM,
    ...
    if is_main:
        wandb.init(
            ...
            name=f'SS2D_v7_titan_ddp_ws{world_size}_BS{BATCH_SIZE}_LR{LEARNING_RATE_ADAM}_EP{NUM_EPOCHS}',
            config={
                'model': 'SS2D-ViT-v7_titan-DDP',
        ...
        wandb.watch(ss2d_decoder, log='gradients', log_freq=200)
    ...
    if run_baseline:
        ...
        baseline = run_val_ddp(ss2d_decoder, val_loader, device, world_size)
        ...
        if is_main:
            ...
            torch.save(ss2d_decoder.module.state_dict(),
                       os.path.join(PATH_FOLDER, 'ss2d_vit_best.pt'))
    elif is_main:
        with open(log_path, 'a') as f:
            f.write(
    ...
    for epoch in epoch_bar:
        ...
        if do_val:
            ...
            val_metrics = run_val_ddp(ss2d_decoder, val_loader, device, world_size)
            ...
            if is_main:
                ...
                if val_metrics['composite'] > best_val_composite:
                    ...
                    torch.save(ss2d_decoder.module.state_dict(), best_ckpt_path)
                    tqdm.write(
            ...
            # broadcast EarlyStop decision to all ranks
        ...
        if is_main and (epoch + 1) % 5 == 0:
            ...
            torch.save(ss2d_decoder.module.state_dict(), ckpt_path)
    ...
    if is_main:
        ...
        _print(f'Best Checkpoint: {os.path.join(PATH_FOLDER, "ss2d_vit_best.pt")}')
        wandb.finish()
