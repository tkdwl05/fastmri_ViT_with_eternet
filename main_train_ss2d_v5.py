"""
SS2D-ViT 학습 스크립트 v5
  - Config:     myConfig_choh_SS2D_model_v5
  - Dataloader: dataloader_h5_v5 (사이즈 필터 완화 + augmentation)
  - Model:      u_choh_model_SS2D_ViT_v4 (변경 없음, dropout 만 config 로 주입)
  - 추가: EarlyStopping (val composite 기준, patience=N val check)
"""
...
def main():
    print('====================================================')
    print(' [SS2D-ViT v5] size-relaxed + flip aug + EarlyStop')
    print('====================================================')

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA GPU 필수. torch.cuda.is_available()가 False.")
    device = torch.device("cuda")
    print(f"Device: {device}")
    print(datetime.datetime.now(pytz.timezone('Asia/Seoul')))
    if not check_env_for_model('ss2d', 'myConfig_choh_SS2D_model_v5', strict=True):
        return
    ...
    # 2. 디코더 (v4 모델, dropout 만 config 로 주입)
    ...
    # 4. 데이터셋 (v5: train augmentation on)
    ...
    wandb.init(
        ...
        name=f'SS2D_v5_BS{BATCH_SIZE}_LR{LEARNING_RATE_ADAM}_EP{NUM_EPOCHS}',
        config={
            'model': 'SS2D-ViT-v5',
            'dropout': DROPOUT,
