"""
SS2D-ViT 학습 스크립트 v6
  - Config:     myConfig_choh_SS2D_model_v6
  - Dataloader: dataloader_h5_v5 (v5 와 동일)
  - Model:      u_choh_model_SS2D_ViT_v4 (v5 와 동일)
  - 변경점 (v5 대비):
      * v5 best ckpt (RESUME_CKPT) 부터 weight 시작
      * val SSIM 측정을 fastMRI 표준 (skimage data_range 기반) 으로 교체
        — u_choh_SSIM 은 val_range=None 시 L=1 로 고정되어 작은 dynamic range
          fastMRI raw RSS 값을 부정확하게 평가했음
      * EarlyStop / best 기준: composite → val_ssim 단일
      * train_best 마다 val 폐지, VAL_EVERY_N_EPOCHS = 5
      * loss 는 v5 그대로 (학습 신호는 일단 유지, 추가 변수 도입 회피)
"""
...
from myConfig_choh_SS2D_model_v6 import *
...
def main():
    print('====================================================')
    print(' [SS2D-ViT v6] resume + skimage SSIM val + SSIM-only EarlyStop')
    print('====================================================')
    ...
    if not check_env_for_model('ss2d', 'myConfig_choh_SS2D_model_v6', strict=True):
        return
    ...
    # 4. 옵티마이저, 손실 (loss 는 v5 그대로 — 학습 신호 유지)
    criterion_l1   = nn.L1Loss()
    criterion_ssim_loss = SSIM().to(device)   # loss 용 (val 평가에는 사용 안 함)
    ...
    wandb.init(
        ...
        name=f'SS2D_v6_resume_BS{BATCH_SIZE}_LR{LEARNING_RATE_ADAM}_EP{NUM_EPOCHS}',
        config={
            'model': 'SS2D-ViT-v6',
    ...
    # 7a. baseline val (resume weight 의 진짜 시작점)
    ...
    # baseline ckpt 도 best 로 저장 (이후 학습이 더 나빠질 경우 대비)
    ...
    for epoch in epoch_bar:
        ...
        for sample in batch_bar:
            ...
            with torch.no_grad():
                ...
                ssim_val_train = 1 - loss_ssim.item()    # custom SSIM (loss 용, 참고치)
